...
`vercel.json` registers `/api/jobs/refresh-live` as a daily Vercel Cron at `05:00 UTC`, which is compatible with the Hobby/free plan minimum interval. Vercel automatically sends `Authorization: Bearer $CRON_SECRET` to cron invocations when `CRON_SECRET` is configured in the project environment.
